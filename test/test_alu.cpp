...
TEST(ALUTest, AddTest) {
    ALU::ALU alu;
    auto result = alu.execute(ALU::ALUOp::ADD, 10, 20);
...
TEST(ALUTest, SubTest) {
    ALU::ALU alu;
    auto result = alu.execute(ALU::ALUOp::SUB, 10, 20);
...
TEST(ALUTest, MulTest) {
    ALU::ALU alu;
    auto result = alu.execute(ALU::ALUOp::MUL, 10, 20);
...
TEST(ALUTest, SllTest) {
    ALU::ALU alu;
    auto result = alu.execute(ALU::ALUOp::SLL, 0b1010, 1);
...
TEST(ALUTest, SrlTest) {
    ALU::ALU alu;
    auto result = alu.execute(ALU::ALUOp::SRL, 0b1010, 1);
...
TEST(ALUTest, SraTest) {
    ALU::ALU alu;
    auto result = alu.execute(ALU::ALUOp::SRA, 0xfffffffffffffffa, 2);
