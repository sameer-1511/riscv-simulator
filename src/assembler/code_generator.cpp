...
std::bitset<32> generateRTypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::R_type_instruction_encoding_map.at(block.getOpcode());
...
std::bitset<32> generateI1TypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::I1_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd     = extractRegisterIndex(block.getRd());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
...
std::bitset<32> generateI2TypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::I2_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd     = extractRegisterIndex(block.getRd());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
    const uint32_t imm    = static_cast<uint32_t>(std::stoi(block.getImm()));
    uint32_t machineCode = 0;
...
std::bitset<32> generateI3TypeMachineCode(const ICUnit &block) {
    const auto &encoding  = InstructionSet::I3_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd     = 0;
...
std::bitset<32> generateSTypeMachineCode(const ICUnit &block) {
    const auto &encoding  = InstructionSet::S_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
    const uint32_t rs2    = extractRegisterIndex(block.getRs2());
    const uint32_t imm    = static_cast<uint32_t>(std::stoi(block.getImm()));
...
std::bitset<32> generateBTypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::B_type_instruction_encoding_map.at(block.getOpcode());
...
std::bitset<32> generateUTypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::U_type_instruction_encoding_map.at(block.getOpcode());
...
std::bitset<32> generateJTypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::J_type_instruction_encoding_map.at(block.getOpcode());
    uint32_t rd  = extractRegisterIndex(block.getRd());
...
std::bitset<32> generateCSRRTypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::CSR_R_type_instruction_encoding_map.at(block.getOpcode());
    uint32_t rd  = extractRegisterIndex(block.getRd());
    uint32_t rs1 = extractRegisterIndex(block.getRs1());
...
std::bitset<32> generateCSRITypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::CSR_I_type_instruction_encoding_map.at(block.getOpcode());
    uint32_t rd   = extractRegisterIndex(block.getRd());
...
std::bitset<32> generateFDRTypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::F_D_R_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd     = extractRegisterIndex(block.getRd());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
    const uint32_t rs2    = extractRegisterIndex(block.getRs2());
    uint32_t machineCode = 0;
...
std::bitset<32> generateFDR1TypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::F_D_R1_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd   = extractRegisterIndex(block.getRd());
    const uint32_t rs1  = extractRegisterIndex(block.getRs1());
    const uint32_t rs2  = extractRegisterIndex(block.getRs2());
...
std::bitset<32> generateFDR2TypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::F_D_R2_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd     = extractRegisterIndex(block.getRd());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
    const uint32_t rm     = static_cast<uint32_t>(block.getRm() & 0b111);
    uint32_t machineCode = 0;
...
std::bitset<32> generateFDR3TypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::F_D_R3_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd     = extractRegisterIndex(block.getRd());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
    uint32_t machineCode = 0;
    machineCode |= (encoding.funct7.to_ulong() << 25);
...
std::bitset<32> generateFDR4TypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::F_D_R4_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd     = extractRegisterIndex(block.getRd());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
    const uint32_t rs2    = extractRegisterIndex(block.getRs2());
...
std::bitset<32> generateFDITypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::F_D_I_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rd     = extractRegisterIndex(block.getRd());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
    const uint32_t imm    = static_cast<uint32_t>(std::stoi(block.getImm()));
    uint32_t machineCode = 0;
...
std::bitset<32> generateFDSTypeMachineCode(const ICUnit &block) {
    const auto &encoding = InstructionSet::F_D_S_type_instruction_encoding_map.at(block.getOpcode());
    const uint32_t rs1    = extractRegisterIndex(block.getRs1());
    const uint32_t rs2    = extractRegisterIndex(block.getRs2());
    const uint32_t imm    = static_cast<uint32_t>(std::stoi(block.getImm()));
    const uint32_t imm_lo = imm & 0b11111;       // bits [4:0]
