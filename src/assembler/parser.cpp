        ...
        if (currentToken().value == "dword") {
            ...
            while (currentToken().type != TokenType::EOF_ 
                ...
                if (currentToken().type == TokenType::STRING)
                    data_buffer_.push_back(std::make_pair(currentToken().value.size(), std::stoull(currentToken().value)));
                data_index_ += currentToken().value.size();
                nextToken();
            }
        }
        else {
            errors_.push_back(ParseError(ErrorType::INVALID_DIRECTIVE, "Invalid directive", currentToken().line, currentToken().column));
            nextToken();
        }
        ...
        if (currentToken().type == TokenType::LABEL) {
            ...
            for (const SyntaxType& syntax : syntaxes) {
                ...
                switch (syntax) {
                            ...
                            if (isValidITypeInstruction(block.getOpcode())) {
                                ...
                                if (-4096 <= imm && imm <= 4095) {
                                    if (imm % 2 == 0) {
                                        ...
                                        errors_.push_back(ParseError(ErrorType::MISALIGNED_IMMEDIATE, "Misaligned immediate value", currentToken().line, currentToken().column));
                                        skipCurrentLine();
                                        break;
                                    }
                                } else {
                    ...
                    default:
                        errors_.push_back(ParseError(ErrorType::INVALID_SYNTAX, "Invalid syntax", currentToken().line, currentToken().column));
                        skipCurrentLine();
                        break;
                }
          /*  
          ...
          */
            ...
            errors_.push_back(ParseError(ErrorType::INVALID_DIRECTIVE, "Invalid token", currentToken().line, currentToken().column));
            skipCurrentLine();
            continue;
        }
    }
...
void Parser::parse() {
    ...
    for (int index : backPatch) {
        ...
        if (symbol_table_.find(block.getImm()) != symbol_table_.end()) {
            ...
            errors_.push_back(ParseError(ErrorType::INVALID_LABEL_REF, "Invalid label", currentToken().line, currentToken().column));
        }
    }
